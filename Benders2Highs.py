...
while iteration <= max_iters:
    ...
    ypq = {(p, q): xp.var(vartype=xp.continuous,name=f"y_{p}_{q}", lb=0) for p in P for q in X}
    zpq = {(p, q): xp.var(vartype=xp.continuous,name=f"z_{p}_{q}",lb=0) for p in X for q in X if p<q}
